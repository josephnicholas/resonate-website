...
There is nothing to build or compile. Edit the HTML/CSS directly and serve the directory:
...
Validation is manual: check rendering at multiple viewport widths (the layout leans heavily on Bootstrap's `col-xl-*` / `col-sm-*` breakpoint pairs, so mobile and desktop diverge significantly). Test mobile (~320px–576px) and desktop (~1200px+) before considering a change complete.
